...
class Arm:
    def __init__(self, IDs): #IDs is a array with the ids of the [shoudler, tilt, pan]
        ...
        self.Shoulder = BC.Joint(IDs[0], 200, 200, 1000)
        self.Tilt = BC.Joint(IDs[1], 200, 200, 800)
        self.Pan = BC.Joint(IDs[2], 512, 0, 1023)
...
class Flipper:
    def __init__ (self, WheelID, JointID): #ID of the seervo that controlls the flipper
        ...
        if self.JointID in [5,8]:
            self.Joint = BC.Joint(self.JointID, 200, 200, 800)
        elif self.JointID in [6,7]:
            self.Joint = BC.Joint(self.JointID, 800, 200, 800)
        self.Joint.start()
    ...
    def JointReset(self):
        if self.JointID in [5,8]:
            self.Joint.moveJoint(200, 999)
            self.Joint.position = 200
        elif self.JointID in [6,7]:
            self.Joint.moveJoint(800, 999)
            self.Joint.position = 800
